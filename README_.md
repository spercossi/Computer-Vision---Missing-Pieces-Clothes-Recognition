...
The notebook is designed to run on **Google Colab** with a GPU runtime (used for the ResNet18 training step).

1. Open the notebook: `notebooks/ProgettoCV_Missing_Pieces.ipynb`
   (https://colab.research.google.com/drive/1gO8p69ofXzcyg5O-TAC730Pev2aND9r0?usp=sharing).
2. Set **Runtime → Change runtime type → GPU** before executing.
3. Run all cells in order (Phase 0 → Phase 6). Phase 0 installs all required dependencies automatically.
4. The dataset is downloaded automatically via `kagglehub` in Phase 1 — no manual download needed (requires a Kaggle account the first time you authenticate).
